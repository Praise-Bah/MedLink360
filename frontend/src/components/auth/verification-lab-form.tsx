...
export function VerificationLabForm() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    console.log("Lab verification submitted", files)
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-8">
            <div className="rounded-lg border border-[#e2e4e5] p-8">
              <div className="rounded-lg border border-[#e2e4e5] p-8 space-y-8">
                ...
                {/* Lab License Upload */}
                <FileUploadSection
                  title="Upload Laboratory Technician License (Front & Back)"
                  onChange={(file) => setFiles({...files, license: file})}
                />

                {/* Lab Logo Upload */}
                <FileUploadSection
                  title="Upload Laboratory Image/Logo (displayed to patients)"
                  onChange={(file) => setFiles({...files, logo: file})}
                />

                {/* Lab Affiliation Upload */}
                <FileUploadSection
                  title="Upload Laboratory affiliation proof"
                  onChange={(file) => setFiles({...files, affiliation: file})}
                />
