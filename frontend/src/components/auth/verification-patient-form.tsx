...
export function VerificationPatientForm() {
  const router = useRouter()
  const [idFile, setIdFile] = useState<File | null>(null)
  const [healthCopyFile, setHealthCopyFile] = useState<File | null>(null)

  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    console.log("Patient verification submitted", { idFile, healthCopyFile })
    ...
    <div className="min-h-screen bg-[#f2f8ff] px-6 py-10">
      <div className="mx-auto flex w-full max-w-[456px] flex-col gap-20 pt-2">
        ...
        <div className="flex items-center gap-2">
          <div className="h-3 w-3 rounded-full bg-[color:var(--brand-primary)]" />
          <div className="h-0.5 w-10 bg-[color:var(--brand-primary)]" />
          <div className="h-3 w-3 rounded-full bg-[color:var(--brand-primary)]" />
        ...
        <div className="flex flex-col gap-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-8">
              ...
              <div className="rounded-lg border border-[#e2e4e5] p-8">
                ...
                <div className="mb-8 flex flex-col gap-0.5">
                  ...
                  <p className="text-[12px] font-light leading-4 text-[#575f6e]">
                    Take Photo of Your ID (Front & Back)
                  </p>
                </div>

                <p className="mb-8 text-[12px] font-light leading-4 text-[#575f6e]">
                  Please make sure there&apos;s enough lighting and that the ID lettering is clear before continuing.
                </p>

                {/* ID Upload Area */}
                <div className="relative mb-8 flex h-[183px] w-full items-center justify-center rounded-[4px] border-2 border-dashed border-[#212529]">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[14px] font-normal leading-5 text-[#242426]">
                    Upload Previous Health Copy
                  </label>
                  <div className="relative flex h-[115px] items-center justify-center rounded-[4px] border border-dashed border-[rgba(0,123,255,0.87)] bg-[#e1e7ef] px-28 py-3">
                    <div className="flex flex-col items-center gap-2 text-center">
                      ...
                      <div className="mt-1 flex items-center gap-1 rounded-[3px] bg-[#f4f4f4] px-2 py-1">
                    ...
                    <input
                      ...
                      accept="image/*,.pdf"
                      multiple
                      onChange={(e) => setHealthCopyFile(e.target.files?.[0] ?? null)}
                      className="absolute inset-0 cursor-pointer opacity-0"
                    />
                  </div>
                  {healthCopyFile && (
                    <p className="text-[12px] text-[#575f6e]">Selected: {healthCopyFile.name}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <button
              type="submit"
              className="flex h-12 items-center justify-center gap-2 rounded border border-[#bbbfc1] bg-white px-6 text-[16px] font-medium capitalize leading-6 text-[#242731]"
            >
              Go next
